...
def run_benchmark():
        ...
        for idx, row in input_data.iterrows():
            try:
                ...
                row_sc4_1 = pd.Series([
                    ...
                    trivia_questions[1],
                    trivia_questions[2],
                    f"{row.iloc[1]}, {row.iloc[2]}, {row.iloc[3]}",# other people's contradicting soft preferences
    ...
    binary_results_df.to_excel(f'eval_results_sc4_binary_{timestamp}.xlsx', index=False)
    neutral_results_df.to_excel(f'eval_results_sc4_neutral_{timestamp}.xlsx', index=False)
